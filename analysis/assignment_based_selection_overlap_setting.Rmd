...
This estimate also accurately recovers all of the factors.
...
Now, let's apply stability selection. Based off of the histograms of the cosine similarity, I chose a threshold of 0.8.
```{r}
set.seed(1)
...
These are the true factors that we were able to recover (the correlation was larger than 0.9):
```{r}
apply(cor(L_est_by_col, sim_data$LL), 1, which.max)[apply(cor(L_est_by_col, sim_data$LL), 1, max) > 0.9]
```
