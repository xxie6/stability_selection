...
This is the correlation between the estiamte and the true loadings:
```{r}
apply(cor(L_est_by_col, sim_data$LL), 1, max)
```

In this case, the method was able to get rid of the extra factors. Something to think about is how to reconcile slight differences in the estimates -- do I just pick one estimate? do I take an average? Taking an average feels intuitive, but for now, I just pick one.
...
The method is also able to recover all four components.
...
The method also recovers all four components.
...
This method returns all four components as well.
...
This method also returns all four components.

# Observations
In this setting, all of the methods were able to recover all four components.
