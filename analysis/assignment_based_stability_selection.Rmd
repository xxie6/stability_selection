...
In this analysis, I try a stability selection method based on the Hungarian Algorithm.
...
Some of these factors are zero vectors. So we should remove these. (It seems like GBCD does not have a check for whether factors are zero. I think this is something that Joon came across once?). Note that removing the zero factors yields two factor estimates with four factors each, meaning the method selected the correct number of factors.
...
I also normalize all of factors to have l2 norm = 1. (Though the cosine similarity is scale-invariant so this should not affect the results).
```{r}
...
Now, we apply stability selection. After analyzing the cosine similarity matrix and histograms, I again chose a similarity threshold of 0.8. Note that in this case, the gap between similar pairs and dissimilar pairs is quite large, so a much lower threshold would still have yielded the same results.
```{r}
set.seed(1)
...
Now, let's apply stability selection. Based off the histograms above, I chose a similarity threshold of 0.8. Again, the gap between similar and dissimilar pairs is quite large, so I could have gone with a lower threshold (I think 0.4 is the lowest I can go without picking up extra factors).
```{r}
set.seed(1)
...
for (i in 1:(length(X_split_by_col)/2)){
  ...
  codesymnmf_fits_by_col[[i]] <- codesymnmf::codesymnmf(S, 8)$H
}
```
...
The gap between similar and dissimilar pairs in these histograms is not as large as that of the EBCD histograms.

Now, let's apply stability selection. Based off the histograms above, I chose a similarity threshold of 0.8. I tested a few other threshold options and I found that 0.7 was the smallest I could go without returning extra factors.
```{r}
set.seed(1)
...
Stability selection with CoDesymNMF is also able to recover all four components. However, the extra factors looked more similar to true signal than the extra factors seen in the previous methods. Therefore a more stringent threshold was needed to not return any extra factors. 
...
This is a plot of the last factor estimate from the first subset:
```{r}
par(mfrow = c(1,2))
for (i in 5){
  plot(gbcd_fits_by_row_F[[1]][,i], ylab = 'Factor Estimate')
...
This is another instance of the method picking out the correct number of factors (after removing the factors which were zeroed out).
...
Now, let's apply stability selection. I again use a similarity threshold of 0.8. (I tested out other threshold options and 0.3 was the smallest threshold which did not return any extra factors).
```{r}
set.seed(1)
L_est_by_col <- stability_selection_post_processing(ebcd_fits_by_col[[1]], ebcd_fits_by_col[[2]], threshold = 0.8)
```
