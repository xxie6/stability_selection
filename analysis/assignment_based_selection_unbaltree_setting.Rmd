---
title: "assignment_based_selection_unbaltree_setting"
author: "Annie Xie"
date: "2025-10-08"
output: 
...
# Unbalanced Tree Data Generation
...
These are plots of the last two factor estimates:
```{r}
par(mfrow = c(1,2))
for(i in 6:7){
  plot(gbcd_fits_by_col_F[[1]][,i], main = paste('factor', i))
}
par(mfrow = c(1,1))
```

The last two factors appear to be essentially zero. Taking this into account, this would get rid of the duplicates in the GBCD loadings estimate.
...
This is a plot of the last factor estimate:
```{r}
for(i in 7){
  plot(gbcd_fits_by_col_F[[2]][,i], main = paste('factor', i))
}
```

The last factor estimate seems to be zero. The last loadings estimate essentially captures the first and fourth populations, which are already captured in two other components. Perhaps the semi-nonnegative matrix factorization viewed the last factor as redundant. 

I will keep the loadings corresponding to zero factors for now. But perhaps this is a post-processing step that should be added to the GBCD output.
...
This estimate doesn't look exactly like the unbalanced tree decomposition I was going for. I think the method prefers a more balanced representation, so it favors a "baseline" which incorporates both the baseline factor and the 3 vs 1 split.
...
```{r}
apply(cor(gbcd_fits_by_row_F[[1]], sim_data$FF), 2, max)
```
...
This is the max correlation for each true factor:
```{r}
apply(cor(gbcd_fits_by_row_F[[2]], sim_data$FF), 2, max)
```
...
For a threshold of 0.6, the method only returns three factors. Two of the loadings correspond to population-specific effects. The third loadings factor captures a combination of the baseline factor and the 3 vs 1 factor -- this is something I've commonly seen in other decompositions.

### Threshold = 0.2

For comparison, I try a lower similarity threshold of 0.2.
```{r}
set.seed(1)
F_est <- stability_selection_post_processing(gbcd_fits_by_row_F[[1]], gbcd_fits_by_row_F[[2]], threshold = 0.2)
```
...
With this threshold, the method returns four factors. The additional factor captures a combination of the 2 vs 2 split and the last population-specific effect.
...
Overall, the estimates do an okay job at recovering the true factors. Visually speaking, the loadings estimates capture some of the tree structure. But the decomposition is not strictly binary; multiple effects seem to be combined into single factors.
...
Two of the loadings capture population-specific effects. However, the other two factors seem to capture numerous effects. Overall, this is not the binary decomposition that we were aiming to find.
...
This estimate effectively uses four factors.
...
This estimate also effectively uses four-five factors.
...
The final estimate contains four factors. Some of the factors contain binary-like components. But overall, this is not the binary decomposition we were hoping to find. This speaks to the struggles CoDesymNMF has in the unbalanced tree setting.
